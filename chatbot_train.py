...
tokenizer = tfds.features.Text.maybe_build_from_corpus(questions + answers, target_vocab_size=2**13)
...
def encoder_layer(units, d_model, num_heads, dropout, name="encoder_layer"):
    ...
    outputs = tf.keras.layers.Dense(
        units=units, activation='relu')(attention)
...
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,monitor="val_loss",save_best_only=True,mode="min"
                                                 verbose=1)


hist = model.fit(dataset, epochs=EPOCHS, callbacks=[
